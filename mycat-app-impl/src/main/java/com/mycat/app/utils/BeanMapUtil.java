...
public class BeanMapUtil {
     ...
     * @throws InvocationTargetException 如果调用属性的 setter 方法失败
     */
    public static Object convertMap2Bean(Object obj, Map map) throws IntrospectionException,
     ...
     * @throws Exception
     */
    public static void validateBean(Object bean, String ruleId) throws Exception {
        Map map = convertBean(bean);
        String rule = SpringUtil.getProperty("Rule." + ruleId);
        if (StringUtil.isEmpty(rule)) {
            throw new BusinessException("校验规则未配置:" + ruleId);
        }
        for (String key : rule.split(",")) {
            Object value = map.get(key);
            if (value == null || StringUtil.isEmpty(String.valueOf(value))) {
                throw new BusinessException("cost_code[" + map.get("costCode") + "]必需字段为空:" + key);
            }
        }
    }
}